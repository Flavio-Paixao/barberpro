...
<head>
  ...
  <title>BarberPro — Profissionals</title>
  <style>
...
<main class="main">
  <div class="topbar">
    <div>
<div class="topbar-title"><span class="glitch" data-text="PROFISSIONALS">BARBEI<span style="color:var(--red)">ROS</span></span></div>      <div class="topbar-sub">// Gestão da equipe</div>
    </div>
    ...
    <div class="grid-3">
      ...
      <div class="barber-card">
        <div class="barber-header">
          ...
          <div><div class="barber-name">{{ $b->nome }}</div><div class="barber-spec">{{ $b->especialidade }}</div></div>
          <label class="toggle" style="margin-left:auto"><input type="checkbox" {{ $b->ativo ? 'checked' : '' }}/><span class="toggle-slider"></span></label>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <div style="background:var(--surface);padding:10px;text-align:center;border-radius:4px">
            <div style="font-family:'Bebas Neue';font-size:20px;color:var(--red)">{{ $b->agendamentos_count }}</div>
            <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Atend. total</div>
          </div>
          <div style="background:var(--surface);padding:10px;text-align:center;border-radius:4px">
            <div style="font-size:11px;color:var(--muted)">{{ $b->hora_inicio }} — {{ $b->hora_fim }}</div>
            <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-top:4px">Horário</div>
          </div>
        </div>
        <span class="badge {{ $b->ativo ? 'badge-green' : 'badge-red' }}">{{ $b->ativo ? '● Ativo' : '● Inativo' }}</span>
      </div>
      @endforeach
    </div>
  </div>

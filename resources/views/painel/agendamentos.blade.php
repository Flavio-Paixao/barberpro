...
<head>
  ...
  <title>StudioPro — Agendamentos</title>
  <style>
...
<main class="main">
  <div class="topbar">
    <div>
      <div class="topbar-title"><span class="glitch" data-text="AGENDAMENTOS">AGENDA<span style="color:var(--gold-light)">MENTOS</span></span></div>
  ...
  <div class="content">
    <div class="card">
      <div class="card-header">
        <div><div class="card-title">Todos os <span style="color:var(--gold-light)">Agendamentos</span></div><div class="card-sub">// Ordenados por data</div></div>
        <form method="GET" action="{{ route('painel.agendamentos') }}" style="display:flex;gap:8px;align-items:center">
          <input type="date" name="data" value="{{ $dataFiltro }}" class="form-input" style="width:160px"/>
          <button type="submit" class="btn btn-blue" style="padding:8px 12px">Filtrar</button>
          @if($dataFiltro)
          <a href="{{ route('painel.agendamentos') }}" class="btn" style="padding:8px 12px;background:#1a1a1a">Limpar</a>
          @endif
        </form>
      </div>
      <table>

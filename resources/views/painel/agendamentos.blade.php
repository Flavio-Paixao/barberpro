...
<head>
  ...
  <title>BarberPro — Agendamentos</title>
  <style>
...
<main class="main">
  <div class="topbar">
    <div>
      <div class="topbar-title"><span class="glitch" data-text="AGENDAMENTOS">AGENDA<span style="color:var(--blue-light)">MENTOS</span></span></div>
      <div class="topbar-sub">// Histórico completo</div>
    </div>
    <a href="{{ route('home') }}" target="_blank" class="btn btn-blue"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> Novo Agendamento</a>
  </div>
  <div class="content">
    <div class="card">
      <div class="card-header"><div><div class="card-title">Todos os <span style="color:var(--blue-light)">Agendamentos</span></div><div class="card-sub">// Ordenados por data</div></div></div>
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="color:var(--muted)">{{ $ag->cliente_telefone }}</td>
            <td>{{ $ag->profissional?->nome }}</td>
            <td>{{ $ag->servico?->nome }}</td>
            <td style="color:var(--green)">R$ {{ number_format($ag->servico?->preco ?? 0, 2, ',', '.') }}</td>
            <td>
              ...
              @elseif($ag->status === 'nao_compareceu') <span class="badge badge-red">Não compareceu</span>
      ...
      <div style="margin-top:20px">{{ $agendamentos->links() }}</div>
    </div>
  </div>
</main>
</body></html>
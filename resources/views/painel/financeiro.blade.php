...
<head>
  ...
  <title>BarberPro — Financeiro</title>
  <style>
...
<main class="main">
  <div class="topbar">
    <div>
      <div class="topbar-title"><span class="glitch" data-text="FINANCEIRO">FINAN<span style="color:var(--blue-light)">CEIRO</span></span></div>
  ...
  <div class="content">
    <div class="grid-3">
      ...
      <div class="metric-card blue"><div class="metric-label">Não Compareceram</div><div class="metric-value" style="font-size:22px;color:var(--red)">{{ $naoCompareceram }}</div><div class="metric-desc">Esse mês</div></div>
    </div>
    <div class="card">
      <div class="card-header"><div><div class="card-title">Faturamento — Últimos <span style="color:var(--blue-light)">6 Meses</span></div><div class="card-sub">// R$ por mês</div></div></div>
      @php $maxFat = collect($meses)->max('faturamento') ?: 1; @endphp
      <div class="bar-chart">
        @foreach($meses as $m)
        <div class="bar-group">
          <span style="font-size:10px;color:var(--blue-light)">R${{ number_format($m['faturamento'], 0, ',', '.') }}</span>

...
<head>
  ...
  <title>BarberPro — Serviços</title>
  <style>
...
<main class="main">
  <div class="topbar">
    <div>
      <div class="topbar-title"><span class="glitch" data-text="SERVIÇOS">SER<span style="color:var(--blue-light)">VIÇOS</span></span></div>
      <div class="topbar-sub">// Preços e duração</div>
    </div>
    <span style="font-size:12px;color:var(--muted)">Gerenciado pelo administrador do sistema</span>
  </div>
  <div class="content">
    <div class="card">
      <div class="card-header"><div><div class="card-title">Serviços <span style="color:var(--blue-light)">Cadastrados</span></div><div class="card-sub">// Gerencie pelo Filament para editar</div></div></div>
      @foreach($servicos as $s)
      <div class="service-item">
        <div style="width:36px;height:36px;background:rgba(220,38,38,.1);border:1px solid rgba(220,38,38,.2);display:flex;align-items:center;justify-content:center;color:var(--red);border-radius:4px"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><line x1="20" y1="4" x2="8.12" y2="15.88"/><line x1="14.47" y1="14.48" x2="20" y2="20"/></svg></div>
        <div class="service-name">{{ $s->nome }}<div style="font-size:11px;color:var(--muted);font-weight:400;margin-top:2px">{{ $s->descricao }}</div></div>
        <div class="service-time">{{ $s->duracao_minutos }} min</div>
        <div style="font-size:11px;color:var(--muted)">{{ $s->agendamentos_count }} agend.</div>
        <div class="service-price">R$ {{ number_format($s->preco, 2, ',', '.') }}</div>
